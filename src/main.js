...
for (let i = 0; i < priceArr.length; i++) {
  var parsed = parseInt(priceArr[i])
  if (parsed <= 399) {
    var priceRangeEl = { id: 1, input: "400", label: "< 400" }
    if (!priceRangeArr.some(obj => obj.input === "400")) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (400 <= parsed && parsed < 500) {
    var priceRangeEl = { id: 2, input: "400-500", label: "400 - 500" }
    if (!priceRangeArr.some(obj => obj.input === "400-500")) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (500 <= parsed && parsed < 600) {
    var priceRangeEl = { id: 3, input: "500-600", label: "500 - 600" }
    var checkVal = obj => obj.input === "500-600";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (600 <= parsed && parsed < 700) {
    var priceRangeEl = { id: 4, input: "600-700", label: "600 - 700" }
    var checkVal = obj => obj.input === "600-700";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (700 <= parsed && parsed < 800) {
    var priceRangeEl = { id: 5, input: "700-800", label: "700 - 800" }
    var checkVal = obj => obj.input === "700-800";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (800 <= parsed && parsed < 900) {
    var priceRangeEl = { id: 6, input: "800-900", label: "800 - 900" }
    var checkVal = obj => obj.input === "800-900";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (900 <= parsed && parsed < 1000) {
    var priceRangeEl = { id: 7, input: "900-1000", label: "900 - 1000" }
    var checkVal = obj => obj.input === "900-1000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (1000 <= parsed && parsed < 1500) {
    var priceRangeEl = { id: 8, input: "1000-1500", label: "1000 - 1500" }
    // var checkVal = ;
    if (!priceRangeArr.some(obj => obj.input === "1000-1500")) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (1500 <= parsed && parsed < 2000) {
    var priceRangeEl = { id: 9, input: "1500-2000", label: "1500 - 2000" }
    var checkVal = obj => obj.input === "1500-2000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (2000 <= parsed && parsed < 2500) {
    var priceRangeEl = { id: 10, input: "2000-2500", label: "2000 - 2500" }
    var checkVal = obj => obj.input === "2000-2500";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (2500 <= parsed && parsed < 10000) {
    var priceRangeEl = { id: 11, input: "2500-10000", label: "2500 - 10000" }
    var checkVal = obj => obj.input === "2500-10000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (10000 <= parsed && parsed < 25000) {
    var priceRangeEl = { id: 12, input: "10000-25000", label: "10000 - 25000" }
    var checkVal = obj => obj.input === "10000-25000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (25000 <= parsed && parsed < 50000) {
    var priceRangeEl = { id: 13, input: "25000-50000", label: "25000 - 50000" }
    var checkVal = obj => obj.input === "25000-50000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (50000 <= parsed && parsed < 75000) {
    var priceRangeEl = { id: 14, input: "50000-75000", label: "50000 - 75000" }
    var checkVal = obj => obj.input === "50000-75000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (75000 <= parsed && parsed < 100000) {
    var priceRangeEl = { id: 15, input: "75000-100000", label: "75000 - 100000" }
    var checkVal = obj => obj.input === "75000-100000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (100000 <= parsed && parsed < 125000) {
    var priceRangeEl = { id: 16, input: "100000-125000", label: "100000 - 125000" }
    var checkVal = obj => obj.input === "100000-125000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (125000 <= parsed && parsed < 150000) {
    var priceRangeEl = { id: 17, input: "125000-150000", label: "125000 - 150000" }
    var checkVal = obj => obj.input === "125000-150000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (150000 <= parsed && parsed < 200000) {
    var priceRangeEl = { id: 18, input: "150000-200000", label: "150000 - 200000" }
    var checkVal = obj => obj.input === "150000-200000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (200000 <= parsed && parsed < 250000) {
    var priceRangeEl = { id: 19, input: "200000-2500000", label: "200000 - 2500000" }
    var checkVal = obj => obj.input === "200000-2500000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (250000 <= parsed && parsed < 300000) {
    var priceRangeEl = { id: 20, input: "250000-300000", label: "250000 - 300000" }
    var checkVal = obj => obj.input === "250000-300000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (300000 <= parsed && parsed < 350000) {
    var priceRangeEl = { id: 21, input: "300000-350000", label: "300000 - 350000" }
    var checkVal = obj => obj.input === "300000-350000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (350000 <= parsed && parsed < 400000) {
    var priceRangeEl = { id: 22, input: "350000-400000", label: "350000 - 400000" }
    var checkVal = obj => obj.input === "350000-400000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (400000 <= parsed && parsed < 450000) {
    var priceRangeEl = { id: 23, input: "400000-450000", label: "400000 - 450000" }
    var checkVal = obj => obj.input === "400000-450000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (450000 <= parsed && parsed < 500000) {
    var priceRangeEl = { id: 24, input: "450000-500000", label: "450000 - 500000" }
    var checkVal = obj => obj.input === "450000-500000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (500000 <= parsed && parsed < 550000) {
    var priceRangeEl = { id: 25, input: "500000-550000", label: "500000 - 550000" }
    var checkVal = obj => obj.input === "500000-550000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (550000 <= parsed && parsed < 600000) {
    var priceRangeEl = { id: 26, input: "550000-600000", label: "550000 - 600000" }
    var checkVal = obj => obj.input === "550000-600000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (600000 <= parsed && parsed < 650000) {
    var priceRangeEl = { id: 27, input: "600000-650000", label: "600000 - 650000" }
    var checkVal = obj => obj.input === "600000-650000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (650000 <= parsed && parsed < 700000) {
    var priceRangeEl = { id: 28, input: "650000-700000", label: "650000 - 700000" }
    var checkVal = obj => obj.input === "650000-700000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (700000 <= parsed && parsed < 750000) {
    var priceRangeEl = { id: 29, input: "700000-750000", label: "700000 - 750000" }
    var checkVal = obj => obj.input === "700000-750000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (750000 <= parsed && parsed < 800000) {
    var priceRangeEl = { id: 30, input: "750000-800000", label: "750000 - 800000" }
    var checkVal = obj => obj.input === "750000-800000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (800000 <= parsed && parsed < 850000) {
    var priceRangeEl = { id: 31, input: "800000-850000", label: "800000 - 850000" }
    var checkVal = obj => obj.input === "800000-850000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (850000 <= parsed && parsed < 900000) {
    var priceRangeEl = { id: 32, input: "850000-900000", label: "850000 - 900000" }
    var checkVal = obj => obj.input === "850000-900000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (900000 <= parsed && parsed < 950000) {
    var priceRangeEl = { id: 33, input: "900000-950000", label: "900000 - 950000" }
    var checkVal = obj => obj.input === "900000-950000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else if (950000 <= parsed && parsed < 1000000) {
    var priceRangeEl = { id: 34, input: "950000-500", label: "950000 - 1000000" }
    var checkVal = obj => obj.input === "950000-500";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
  else {
    var priceRangeEl = { id: 35, input: "1000000", label: "> 1000000" }
    var checkVal = obj => obj.input === "1000000";
    if (!priceRangeArr.some(checkVal)) {
      priceRangeArr.push(priceRangeEl)
    }
  }
...
priceRangeArr.sort((a, b) => a.id - b.id);
var priceRangeHtml = priceRangeArr.map(priceRange => `<div class="filter"><div class="filter_check"><label for="${priceRange.input}" class="filter_label"><input type="checkbox" name="prices" value="${priceRange.input}" id="${priceRange.input}" class="filter_icon"><p class="filter_text">${priceRange.label}</p></label></div></div>`).join(' ')
document.querySelectorAll('.filter_wrapper.price')[0].innerHTML = priceRangeHtml

function checkboxPrices(checkedElement) {
  checkedLocations.push(checkedElement.value)
  ...
  Object.entries(propertyCards).forEach(propCard => {
    ...
    checkedLocations.forEach(checkedLocation => {
      if (value.classList.contains(`${checkedLocation}`)) {
        value.parentElement.parentElement.style.display = "block"
      }
    })
  })
}
function uncheckboxPrices(checkedElement) {
  Object.entries(propertyCards).forEach(propCard => {
    const [key, value] = propCard;
    if (value.classList.contains(`${checkedElement.value}`)) {
      value.parentElement.parentElement.style.display = "none"
    }
  })
  var index = checkedLocations.indexOf(checkedElement.value)
  if (index > -1) {
...
for (let i = 0; i < allPrices.length; i++) {
  allPrices[i].addEventListener('change', (event) => {
    if (event.currentTarget.checked) {
      ...
      checkboxPrices(event.currentTarget)
    } else {
      uncheckboxPrices(event.currentTarget)
    }
  })
}